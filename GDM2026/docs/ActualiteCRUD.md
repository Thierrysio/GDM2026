...
Le corps doit contenir les champs attendus par le backend (par exemple titre, description). L'ajout d'une image reste optionnel :
depuis l'application mobile, la bibliothèque admin affiche uniquement les ressources de la catégorie `image` avec un `Content-Type`
de la forme `image/...` et propose d'en sélectionner une (ou aucune). Exemple minimal avec des clés génériques :
...
La clé `image` illustrée ci-dessus est facultative : omettez-la si aucune ressource `image/...` n'est associée à l'actualité.
